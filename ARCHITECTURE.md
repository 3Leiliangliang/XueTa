...
| 维度 | 当前状态 |
| --- | --- |
| 前端 | 页面原型较完整，但多数页面尚未全面接入真实 API |
| 后端 | 业务模块较完整，接口、模型、服务层已经形成骨架 |
| 聊天 | 已有会话和消息持久化，回答为占位式逻辑 |
| 知识库 | 已有文档、切块和关键词检索，向量检索待接入 |
| 练习 | 已有生成、评分、错题与进度回流，规则式实现为主 |
| 观测与异步 | Redis、Langfuse、SSE 能力预留，尚未完全落地 |
...
1. 补齐 Alembic 迁移脚本
2. 把前端核心页面逐步切到真实 API
3. 为聊天模块接入真实大模型和 SSE 输出
4. 为知识库接入 Embedding、pgvector 和召回重排
5. 为练习模块接入更强的 AI 出题与评分能力
6. 接入 Redis 异步任务与 Langfuse 调用链观测
