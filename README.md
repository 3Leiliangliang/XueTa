...
- 前端：除首页/关于等展示页外，`translate / qa / note / planning / desktop / kb / practice / progress / profile / auth` 等核心页面已接入真实后端 API，主流程可联调运行。
- 后端：已完成认证、规划、笔记、聊天、翻译、知识库、练习、进度、桌面布局、文件上传与任务队列等模块的接口、模型和服务层，具备较完整的业务骨架。
- AI 能力：已接入 `OpenAI`（含 fallback），并实现基础流式回答、文本抽取、向量检索与混合召回；在质量优化、异步化和观测层面仍有提升空间。
...
- `Redis / pgvector / LlamaIndex / Langfuse` 已完成依赖与基础接入，其中任务队列、向量检索与调用观测已具备可用基础能力
...
- 已接入 Alembic，并提供 baseline 迁移脚本；生产环境建议固定使用迁移流程而非 `create_all()`
...
- 可以先验证产品结构是否合理
- 可以先让前后端联调与数据库设计稳定下来
- 后续继续增强 OpenAI/兼容网关、Embedding、RAG、链路观测时，不需要推翻整体架构
...
| 用户 | 已完成当前用户查询与资料更新 |
| 学习规划 | 已完成目标、任务 CRUD，AI 计划生成结果可落库为任务 |
| 笔记 | 已完成笔记本、笔记、待办、总结相关接口 |
| 聊天 | 已完成会话、消息、反馈持久化与 SSE 流式接口，支持模型与 fallback 双通道 |
| 知识库 | 已完成知识库、文档、切块、关键词/向量混合检索 |
| 练习 | 已完成练习集生成、作答提交、评分、错题沉淀与进度回流 |
| 进度 | 已完成学习记录、掌握度、复习计划与概览统计 |
| 桌面布局 | 已完成布局保存、读取、列表、更新、删除 |
| 文件 | 已完成上传、列表、详情、下载、删除 |
| 任务队列 | 已完成 Redis 队列、worker、任务注册与状态查询接口（demo 任务可运行） |

## 后续建议

如果你准备继续完善这个项目，建议优先按以下顺序推进：

1. 统一编码与文档一致性（UTF-8、乱码修复、README/架构文档同步）
2. 完成前端 token 自动刷新与会话恢复链路
3. 将文档解析、OCR、embedding 等重任务迁移到 Redis 队列
4. 强化 RAG（重排、引用展示、召回质量优化）
5. 升级练习生成与评分链路（规则 + 模型混合）
6. 完善 Langfuse 全链路观测
7. 补充前端页面级测试、E2E 与 CI 流程
...
## Docker 全栈运行（前后端 + Postgres + Redis + Worker + Langfuse Cloud）

### 1. 首次准备

项目根目录已提供 `docker-compose.yml`，并约定后端容器环境变量来自 `backend/.env.docker`。
...
```powershell
Copy-Item backend\.env.docker.example backend\.env.docker -Force
```
...
如果要启用 Langfuse Cloud 观测，请在 `backend/.env.docker` 中配置：

- `LANGFUSE_PUBLIC_KEY`
- `LANGFUSE_SECRET_KEY`
- `LANGFUSE_HOST`（默认 `https://cloud.langfuse.com`）

如果你所在网络无法访问 Docker Hub，可额外配置镜像覆盖：

- 将 `./.env.docker.images.example` 复制为项目根目录 `./.env`
- 按网络环境调整镜像变量（`POSTGRES_IMAGE`、`REDIS_IMAGE`、`BACKEND_PYTHON_IMAGE`、`WEB_NODE_IMAGE`）
...
1. `docker compose ps` 中 `postgres` 和 `redis` 应为 `healthy`。
2. 打开 `http://localhost:8000/api/v1/health` 应返回健康状态。
3. 打开 `http://localhost:5173` 后应可正常登录并调用 API。
4. 配置 Langfuse key 后，触发聊天/翻译/总结/embedding 调用，应可在 Langfuse Cloud 看到 trace。

### 5. 常见问题排查

- 前端请求不到后端：
  检查 `web` 服务环境变量 `VITE_API_BASE_URL` 是否为 `http://localhost:8000/api/v1`。
- 浏览器出现 CORS 错误：
  检查 `backend/.env.docker` 中 `CORS_ORIGINS` 是否包含 `http://localhost:5173`。
- 后端启动失败并提示数据库连接错误：
  检查 `DATABASE_URL` 主机名是否为 `postgres`（容器内地址），不是 `localhost`。
- Worker 不消费任务：
  检查 `worker` 容器日志，并确认 `REDIS_URL=redis://redis:6379/0`。
- 拉取镜像超时（`registry-1.docker.io` 无法连接）：
  当前 `docker-compose.yml` 默认使用了 `m.daocloud.io` 代理前缀。如果你所在网络仍无法访问，可在项目根目录创建 `.env` 覆盖镜像：
  `POSTGRES_IMAGE=...`、`REDIS_IMAGE=...`、`BACKEND_PYTHON_IMAGE=...`、`WEB_NODE_IMAGE=...`。
  然后重新执行 `docker compose up -d --build`。
- Docker 提示 `open C:\\Users\\...\\.docker\\config.json: Access is denied`：
  修复该文件权限（确保当前 Windows 用户可读写），或临时移走损坏/不可读的 `~/.docker/config.json` 后重试。
